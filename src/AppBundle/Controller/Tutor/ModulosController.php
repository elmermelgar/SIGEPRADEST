...
class ModulosController extends SecurityController
{
    ...
    public function notasAlumnoAction($id1, $id2, Request $request)
    {
        ...
        $sql = "select e.* from evaluacion e, inscripcion_curso ic, modulos m 
            where e.id_modulo=m.id_modulo and ic.id_curso=m.id_curso and m.id_modulo=$id1 and ic.id_dc=$id2 
            and e.id_evaluacion not in (select id_evaluacion from nota where id_modulo=$id1 and id_dc=$id2)
            ;";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        $eva = $stmt->fetchAll();
